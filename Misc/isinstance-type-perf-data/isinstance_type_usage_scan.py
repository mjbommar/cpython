...
class UsageVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if func_name in {"isinstance", "issubclass"} and len(node.args) >= 2:
            ...
            relevant = [target for target in targets if self._is_relevant_target(target)]
            if relevant:
                self.instancecheck_calls.append(
                    {"lineno": node.lineno, "kind": func_name, "targets": relevant}
                )
        self.generic_visit(node)
    ...
    def _is_relevant_target(self, target: str) -> bool:
        ...
        if target.startswith("collections.abc."):
            return True
        return target.startswith("typing.") and target.endswith("Protocol")
