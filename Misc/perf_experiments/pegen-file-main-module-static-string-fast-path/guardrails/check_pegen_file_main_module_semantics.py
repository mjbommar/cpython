...
def run(python: Path, script: Path, *extra: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    env.setdefault("PYTHONHASHSEED", "0")
    return subprocess.run(
        [str(python), *extra, "-S", "-B", str(script)],
        env=env,
        text=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="pegen-file-main-guardrail-") as tmp:
        root = Path(tmp)
        ...
        invalid_escape = root / "invalid_escape.py"
        invalid_escape.write_text("x = '\\q'\n", encoding="utf-8")
        proc = run(python, invalid_escape, "-Wd")
        assert proc.returncode == 0, proc.stderr
        assert "invalid escape sequence" in proc.stderr, proc.stderr

    print("pegen file main module semantics: ok")
    return 0
