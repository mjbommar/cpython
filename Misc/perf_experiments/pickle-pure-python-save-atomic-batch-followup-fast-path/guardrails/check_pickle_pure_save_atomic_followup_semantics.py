#!/usr/bin/env python3
"""Guardrails for pickle save-side atomic batch follow-up candidates."""
...
class AtomicPersistentPickler(pickle._Pickler):
    def persistent_id(self, obj):
        if type(obj) is str and obj.endswith("17"):
            return f"str:{obj}"
        if type(obj) is bytes and obj.endswith(b"17"):
            return f"bytes:{obj.decode()}"
        if type(obj) is bool and obj:
            return "bool:true"
        return None
...
def main() -> None:
    baseline = {
        ...
        "persistent": capture_dumps(AtomicPersistentPickler),
    }
    ...
    print("pickle pure save atomic follow-up guardrails: ok")
