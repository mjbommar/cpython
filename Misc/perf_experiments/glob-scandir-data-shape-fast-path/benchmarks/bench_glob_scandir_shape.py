#!/usr/bin/env python3
"""Focused benchmark for glob scandir data-shape ideas."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--variant",
        choices=("baseline", "recursive_only", "both_selectors"),
        default="baseline",
    )
    ...
    tmp = pathlib.Path(tempfile.mkdtemp(prefix="perf-glob-shape-"))
    try:
        build_tree(tmp)
        p = pathlib.Path(tmp)
        with Variant(ns.variant):
            results = {
                ...
                "G6_glob_wildcard_all": measure(
                    "glob_wildcard_all",
                    lambda: list(glob.iglob(str(tmp / "pkg0" / "*"))),
                    loops=ns.loops * 2,
                    repeat=ns.repeat,
                ),
                "G7_glob_wildcard_py": measure(
                    "glob_wildcard_py",
                    lambda: list(glob.iglob(str(tmp / "pkg0" / "*.py"))),
                    loops=ns.loops * 2,
                    repeat=ns.repeat,
                ),
                "G8_pathlib_glob_wildcard_all": measure(
                    "pathlib_glob_wildcard_all",
                    lambda: list((p / "pkg0").glob("*")),
                    loops=ns.loops * 2,
                    repeat=ns.repeat,
                ),
            }
