#!/usr/bin/env python3
"""Guardrails for glob scandir data-shape candidates."""
...
def build_tree(root: pathlib.Path) -> None:
    ...
    (root / ".hidden").mkdir()
    (root / ".hidden" / "secret.py").write_text("s = 4\n")
...
def collect(root: pathlib.Path):
    ...
    return {
        ...
        "glob_wildcard": sorted(glob.iglob(str(root / "pkg" / "*"))),
        ...
        "pathlib_glob_wildcard": sorted(str(x) for x in (p / "pkg").glob("*")),
    }


def main() -> None:
    tmp = pathlib.Path(tempfile.mkdtemp(prefix="guard-glob-shape-"))
    try:
        build_tree(tmp)
        baseline = collect(tmp)
        install_candidate("both_selectors")
        try:
        ...
        shutil.rmtree(tmp)
    print("glob scandir shape guardrails: ok")
