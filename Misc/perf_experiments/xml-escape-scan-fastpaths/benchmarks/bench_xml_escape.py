#!/usr/bin/env python3
"""Focused benchmark for ElementTree XML escaping paths."""
...
def _build_clean_tree(width: int, depth: int) -> ET.Element:
...
def _build_escape_tree(width: int, depth: int) -> ET.Element:
...
CLEAN_TREE = _build_clean_tree(width=5, depth=3)
ESCAPE_TREE = _build_escape_tree(width=5, depth=3)
...
def _profile(iterations: int) -> str:
    ...
    profile.disable()
    out = io.StringIO()
    stats = pstats.Stats(profile, stream=out).strip_dirs().sort_stats("tottime")
